...
public class Userdata extends Api implements Serializable {
		...
		if (!fakeData) {
			try {
				...
				JSONArray array = temp.getJSONObject(
						"activities-log-distance-intraday").getJSONArray(
						"dataset");
				Distancevalue = new int[array.length()];
				for (int i = 0; i < array.length(); i++) {
					Distancevalue[i] = array.getJSONObject(i).getInt("value");
				}
	...
	public int[] getDistanceHourGraph() {
	...
	public void setCaloriesHourGraph() {
		if (!fakeData) {
			try {
				...
				JSONArray array = temp.getJSONObject(
						"activities-log-calories-intraday").getJSONArray(
						"dataset");
	...
	public void setHeartRateHourGraph() {
		if (!fakeData) {
			try {
				...
				int numItem = (int) Math.floor(array.length() / 15);
				HeartRatevalue = new int[numItem];
				for (int i = 0; i < numItem; i++) {
					HeartRatevalue[i] = array.getJSONObject(i * 15).getInt(
							"value");
				}
			} catch (NullPointerException e) {
				JOptionPane.showMessageDialog(null,
