...
public class Api {
	...
	/**
	 * getInfo method used to get information using API
	 * @param requestedURL the request URL
	 * @return JSONObject get from the request
	 */
	public static JSONObject getInfo(String requestedURL) {
		...
		try {
			...
			FileReader fileReader = new FileReader(
					"src/main/resources/Team4Credentials.txt");
		...
		FitbitOAuth20ServiceImpl service = (FitbitOAuth20ServiceImpl) new ServiceBuilder()
				...
				.apiSecret(apiSecret).callback("http://localhost:8080")
				.scope(scope).grantType("authorization_code")
				.build(FitbitApi20.instance());

		// The access token contains everything you will need to authenticate
		// your requests
		// It can expire - at which point you will use the refresh token to
		// refresh it
		// See: https://dev.fitbit.com/docs/oauth2/#refreshing-tokens
		// I have authenticated and given you the contents of the response to
		// use
		...
		requestUrl = requestUrlPrefix + requestedURL; // "activities/floors/date/2016-01-07/1d/1min/time/19:15/19:30.json";
		...
		try {
			...
			fileWriter = new FileWriter("src/main/resources/Team4Tokens.txt");
